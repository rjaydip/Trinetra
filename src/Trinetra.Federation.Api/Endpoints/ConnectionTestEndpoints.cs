...
public static class ConnectionTestEndpoints
{
    ...
    private static async Task<Results<Accepted<ConnectionTestAccepted>, NotFound, ProblemHttpResult>>
        StartAsync(
            Guid id,
            ConnectorTargetRepository targets,
            ConnectionTestRepository tests,
    ...
    private static async Task<Results<Ok<ConnectionTestResult>, NotFound>> ResultAsync(
        Guid id, Guid testId, ConnectorTargetRepository targets, ConnectionTestRepository tests,
    ...
    private static async Task<Results<Ok<IReadOnlyList<ConnectionTestSummary>>, NotFound>> ListAsync(
        Guid id, ConnectorTargetRepository targets, ConnectionTestRepository tests,
        HttpContext http, CancellationToken ct)
    {
        var caller = CallerContextFactory.From(http);
        caller.Require("vms.read");
